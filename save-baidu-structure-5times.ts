...
async function saveBaiduStructure() {
  for (let i = 1; i <= 5; i++) {
    console.log(`第 ${i} 次访问...`);
    ...
    try {
      ...
      const filename = `baidu-structure-${i}-${timestamp}.txt`;
      
      fs.writeFileSync(filename, structure.simplified, 'utf-8');
...
saveBaiduStructure().catch(console.error);